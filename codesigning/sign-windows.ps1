# sign-windows.ps1
#
# Semneaza un executabil Windows cu certificatul Self-Signed COMUN GDC
# incarcat ca secret CI (vezi README-windows.md + CLAUDE.md Regula 34).
# Apelat din .github/workflows/build-windows.yml, DOAR cand secretele
# exista - fara ele, build-ul continua nesemnat (fluxul actual, neschimbat).
#
...
# Fix cunoscut (vezi CGConvertor, 2026-09-06): "The input is not a valid
# Base-64 string" - secretul GitHub poate ajunge cu spatii/newline-uri
# parazite (copiere din terminal/caseta web), care strica decodarea
# Base64. Curatam orice caracter alb (spatiu, tab, \r, \n) inainte de
...
try {
    ...
    # NU folosim "signtool verify /pa" - valideaza LANTUL COMPLET de
    # incredere (pana la un certificat radacina de incredere pe MASINA
    # care verifica) - pe runner-ul CI (o masina Windows noua, temporara),
    # certificatul self-signed comun GDC NU e importat in Trusted Root,
    # deci verificarea de incredere esueaza mereu aici, chiar daca
    # semnarea a reusit perfect (vezi CGConvertor, aceeasi cauza gasita
    # la primul test real). Verificam DOAR ca fisierul are efectiv o
    # semnatura Authenticode atasata (Get-AuthenticodeSignature, fara sa
    # cerem lant de incredere) - asta prinde in continuare o semnare care
    # esueaza silentios, fara sa ceara imposibilul.
